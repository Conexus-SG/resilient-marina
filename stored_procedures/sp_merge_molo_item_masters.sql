...
BEGIN
    -- Insert new records
    INSERT INTO DW_MOLO_ITEM_MASTERS (
        ...
        MARINA_LOCATION2_ID, PEDESTAL_ID, PEDESTAL1_ID, QB_ITEM_ID, XERO_ITEM_ID, 
        BARCODE, DISTRIBUTE_TO_OWNERS, FUEL_CLOUD_PRODUCT_ID, HASH_ID, 
        REQUIRES_AGE_VERIFICATION, MINIMUM_AGE, CREATION_DATE_TIME, CREATION_ASPNET_USER_ID, 
        RECORD_STATUS_ID, UPDATE_HASH, SUBLET_ITEM, INTERNAL_REVENUE_XERO_ACCOUNT_ID, 
        INTERNAL_COGS_XERO_ACCOUNT_ID, WIP_XERO_ACCOUNT_ID, INVENTORY_REVALUATION_ID, 
        MARINA_LOCATION6_ID, FINALE_PRODUCT_URL, REVENUE_GL_CODE, COGS_GL_CODE, 
        INVENTORY_GL_CODE, AR_GL_CODE, SALES_TAX_GL_CODE, ONLY_USE_LAST2_AVERAGE, 
        DEFERRED_REVENUE_RECOGNITION, DEFERRED_RECOGNITION_GL_CODE, TRACKING_CODE, 
        ADD_DESCRIPTION_TO_INVOICE_NOTE, MARINA_LOCATION7_ID, IGNORE_INVENTORY_QOH, 
        QOH_COMMITTED, QOH_ON_ORDER, ALLOW_TOTAL_PRICE_ENTRY, MARINA_LOCATION9_ID, 
        ALLOW_POSTING_TO_NON_INCOME_ACCOUNTS, ORDER_COLUMN, ENABLE_NEGATIVE_INVENTORY, WIP,
        DW_LAST_INSERTED, DW_LAST_UPDATED
    )
    SELECT 
        src.ID, src.NAME, src.AMOUNT, src.ITEM_CHARGE_METHOD_ID, src.TAXABLE, src.AVAILABLE_AS_ADD_ON, 
        src.MARINA_LOCATION_ID, src.PRICE, src.TAX, src.SINGLE, src.CHARGE_CATEGORY, src.AMOUNT_IS_DECIMAL, 
        src.NUMBER_OF_DECIMALS, src.ITEM_SHORT_NAME, src.ITEM_CODE, src.TRACKED_INVENTORY, 
        src.QUANTITY_ON_HAND, src.PURCHASE_PRICE, src.FIRST_TRACKING_CATEGORY, src.SECOND_TRACKING_CATEGORY, 
        src.XERO_ID, src.SALE_FREQUENCY, src.LOW_QUANTITY_WARNING, src.MARINA_LOCATION1_ID, 
        src.MARINA_LOCATION2_ID, src.PEDESTAL_ID, src.PEDESTAL1_ID, src.QB_ITEM_ID, src.XERO_ITEM_ID, 
        src.BARCODE, src.DISTRIBUTE_TO_OWNERS, src.FUEL_CLOUD_PRODUCT_ID, src.HASH_ID, 
        src.REQUIRES_AGE_VERIFICATION, src.MINIMUM_AGE, src.CREATION_DATE_TIME, src.CREATION_ASPNET_USER_ID, 
        src.RECORD_STATUS_ID, src.UPDATE_HASH, src.SUBLET_ITEM, src.INTERNAL_REVENUE_XERO_ACCOUNT_ID, 
        src.INTERNAL_COGS_XERO_ACCOUNT_ID, src.WIP_XERO_ACCOUNT_ID, src.INVENTORY_REVALUATION_ID, 
        src.MARINA_LOCATION6_ID, src.FINALE_PRODUCT_URL, src.REVENUE_GL_CODE, src.COGS_GL_CODE, 
        src.INVENTORY_GL_CODE, src.AR_GL_CODE, src.SALES_TAX_GL_CODE, src.ONLY_USE_LAST2_AVERAGE, 
        src.DEFERRED_REVENUE_RECOGNITION, src.DEFERRED_RECOGNITION_GL_CODE, src.TRACKING_CODE, 
        src.ADD_DESCRIPTION_TO_INVOICE_NOTE, src.MARINA_LOCATION7_ID, src.IGNORE_INVENTORY_QOH, 
        src.QOH_COMMITTED, src.QOH_ON_ORDER, src.ALLOW_TOTAL_PRICE_ENTRY, src.MARINA_LOCATION9_ID, 
        src.ALLOW_POSTING_TO_NON_INCOME_ACCOUNTS, src.ORDER_COLUMN, src.ENABLE_NEGATIVE_INVENTORY, src.WIP,
        SYSTIMESTAMP, SYSTIMESTAMP
    FROM STG_MOLO_ITEM_MASTERS src
    WHERE NOT EXISTS (SELECT 1 FROM DW_MOLO_ITEM_MASTERS tgt WHERE tgt.ID = src.ID);
    
    p_inserted_count := SQL%ROWCOUNT;
    
    -- Update existing records (keeping same complex WHERE clause for change detection)
    ...
    WHEN MATCHED THEN
        ...
        WHERE 
            ...
            OR NVL(tgt.AVAILABLE_AS_ADD_ON, -1) != NVL(src.AVAILABLE_AS_ADD_ON, -1)
            ...
            OR NVL(tgt.SINGLE, -1) != NVL(src.SINGLE, -1)
            ...
            OR NVL(tgt.TRACKED_INVENTORY, -1) != NVL(src.TRACKED_INVENTORY, -1)
            ...
            OR NVL(tgt.SALE_FREQUENCY, -1) != NVL(src.SALE_FREQUENCY, -1)
            ...
            OR NVL(tgt.DISTRIBUTE_TO_OWNERS, -1) != NVL(src.DISTRIBUTE_TO_OWNERS, -1)
            OR NVL(tgt.FUEL_CLOUD_PRODUCT_ID, '~') != NVL(src.FUEL_CLOUD_PRODUCT_ID, '~')
            OR NVL(tgt.HASH_ID, '~') != NVL(src.HASH_ID, '~')
            OR NVL(tgt.REQUIRES_AGE_VERIFICATION, -1) != NVL(src.REQUIRES_AGE_VERIFICATION, -1)
            OR NVL(tgt.MINIMUM_AGE, -1) != NVL(src.MINIMUM_AGE, -1)
            ...
            OR NVL(tgt.CREATION_ASPNET_USER_ID, '~') != NVL(src.CREATION_ASPNET_USER_ID, '~')
            ...
            OR NVL(tgt.SUBLET_ITEM, -1) != NVL(src.SUBLET_ITEM, -1)
            ...
            OR NVL(tgt.ONLY_USE_LAST2_AVERAGE, -1) != NVL(src.ONLY_USE_LAST2_AVERAGE, -1)
            OR NVL(tgt.DEFERRED_REVENUE_RECOGNITION, -1) != NVL(src.DEFERRED_REVENUE_RECOGNITION, -1)
            ...
            OR NVL(tgt.ADD_DESCRIPTION_TO_INVOICE_NOTE, -1) != NVL(src.ADD_DESCRIPTION_TO_INVOICE_NOTE, -1)
            OR NVL(tgt.MARINA_LOCATION7_ID, -1) != NVL(src.MARINA_LOCATION7_ID, -1)
            OR NVL(tgt.IGNORE_INVENTORY_QOH, -1) != NVL(src.IGNORE_INVENTORY_QOH, -1)
            OR NVL(tgt.QOH_COMMITTED, -9999999) != NVL(src.QOH_COMMITTED, -9999999)
            OR NVL(tgt.QOH_ON_ORDER, -9999999) != NVL(src.QOH_ON_ORDER, -9999999)
            ...
            OR NVL(tgt.ORDER_COLUMN, -1) != NVL(src.ORDER_COLUMN, -1)
            OR NVL(tgt.ENABLE_NEGATIVE_INVENTORY, -1) != NVL(src.ENABLE_NEGATIVE_INVENTORY, -1)
            OR NVL(tgt.WIP, -9999999) != NVL(src.WIP, -9999999);
    
    p_updated_count := SQL%ROWCOUNT;
    COMMIT;
    
    DBMS_OUTPUT.PUT_LINE('DW_MOLO_ITEM_MASTERS: Inserted ' || p_inserted_count || ', Updated ' || p_updated_count || ' records');
