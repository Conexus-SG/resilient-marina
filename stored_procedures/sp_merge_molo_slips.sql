...
BEGIN
    ...
    SELECT 
        src.ID, src.NAME, src.TYPE, src.RECOMMENDED_LOA, src.RECOMMENDED_BEAM, src.RECOMMENDED_DRAFT, src.RECOMMENDED_AIR_DRAFT, src.MAXIMUM_LOA, src.MAXIMUM_BEAM, src.MAXIMUM_DRAFT, src.MAXIMUM_AIR_DRAFT, src.MARINA_LOCATION_ID, src.PIER_ID, src.STATUS, src.START_DATE, src.END_DATE, src.DO_NOT_COUNT_IN_OCCUPANCY, src.ACTIVE, src.CREATION_DATE_TIME, src.CREATION_USER, src.SLIP_TYPE_ID, src.PAYMENT_PROCESSING_FEE, src.MANAGEMENT_FEE, src.OWNER_ID, src.PAYMENT_PROCESSING_FEE_TYPE_ID, src.MANAGEMENT_FEE_TYPE_ID, src.OVERRIDE_OCCUPANCY_LOA, src.HASH_ID, src.MAINTENANCE_FEE, src.SVG_ID, src.ASSESSMENT, src.LOAN, src.ORDER_COLUMN, src.SIGN_NAME, src.MAX_WEIGHT, src.MAX_REVENUE, src.TRANSIENT_PRICE_ID, src.SEASONAL_PRICE_ID,
            SYSTIMESTAMP,
            SYSTIMESTAMP
    FROM STG_MOLO_SLIPS src
    WHERE NOT EXISTS (
        SELECT 1 FROM DW_MOLO_SLIPS tgt WHERE tgt.ID = src.ID
    );
    
    p_inserted_count := SQL%ROWCOUNT;
    
    -- Update existing records where data has changed
    UPDATE DW_MOLO_SLIPS tgt
    SET (
        tgt.NAME, tgt.TYPE, tgt.RECOMMENDED_LOA, tgt.RECOMMENDED_BEAM, tgt.RECOMMENDED_DRAFT, tgt.RECOMMENDED_AIR_DRAFT, tgt.MAXIMUM_LOA, tgt.MAXIMUM_BEAM, tgt.MAXIMUM_DRAFT, tgt.MAXIMUM_AIR_DRAFT, tgt.MARINA_LOCATION_ID, tgt.PIER_ID, tgt.STATUS, tgt.START_DATE, tgt.END_DATE, tgt.DO_NOT_COUNT_IN_OCCUPANCY, tgt.ACTIVE, tgt.CREATION_DATE_TIME, tgt.CREATION_USER, tgt.SLIP_TYPE_ID, tgt.PAYMENT_PROCESSING_FEE, tgt.MANAGEMENT_FEE, tgt.OWNER_ID, tgt.PAYMENT_PROCESSING_FEE_TYPE_ID, tgt.MANAGEMENT_FEE_TYPE_ID, tgt.OVERRIDE_OCCUPANCY_LOA, tgt.HASH_ID, tgt.MAINTENANCE_FEE, tgt.SVG_ID, tgt.ASSESSMENT, tgt.LOAN, tgt.ORDER_COLUMN, tgt.SIGN_NAME, tgt.MAX_WEIGHT, tgt.MAX_REVENUE, tgt.TRANSIENT_PRICE_ID, tgt.SEASONAL_PRICE_ID,
        tgt.DW_LAST_UPDATED
    ) = (
        SELECT 
            src.NAME, src.TYPE, src.RECOMMENDED_LOA, src.RECOMMENDED_BEAM, src.RECOMMENDED_DRAFT, src.RECOMMENDED_AIR_DRAFT, src.MAXIMUM_LOA, src.MAXIMUM_BEAM, src.MAXIMUM_DRAFT, src.MAXIMUM_AIR_DRAFT, src.MARINA_LOCATION_ID, src.PIER_ID, src.STATUS, src.START_DATE, src.END_DATE, src.DO_NOT_COUNT_IN_OCCUPANCY, src.ACTIVE, src.CREATION_DATE_TIME, src.CREATION_USER, src.SLIP_TYPE_ID, src.PAYMENT_PROCESSING_FEE, src.MANAGEMENT_FEE, src.OWNER_ID, src.PAYMENT_PROCESSING_FEE_TYPE_ID, src.MANAGEMENT_FEE_TYPE_ID, src.OVERRIDE_OCCUPANCY_LOA, src.HASH_ID, src.MAINTENANCE_FEE, src.SVG_ID, src.ASSESSMENT, src.LOAN, src.ORDER_COLUMN, src.SIGN_NAME, src.MAX_WEIGHT, src.MAX_REVENUE, src.TRANSIENT_PRICE_ID, src.SEASONAL_PRICE_ID,
            SYSTIMESTAMP
        FROM STG_MOLO_SLIPS src
        WHERE tgt.ID = src.ID
    )
    WHERE EXISTS (
        SELECT 1 FROM STG_MOLO_SLIPS src
        WHERE tgt.ID = src.ID
        AND (
            ...
            OR NVL(tgt.PAYMENT_PROCESSING_FEE, -9999999) != NVL(src.PAYMENT_PROCESSING_FEE, -9999999)
            OR NVL(tgt.MANAGEMENT_FEE, -9999999) != NVL(src.MANAGEMENT_FEE, -9999999)
            ...
            OR NVL(tgt.MAINTENANCE_FEE, -9999999) != NVL(src.MAINTENANCE_FEE, -9999999)
            OR NVL(tgt.SVG_ID, '~') != NVL(src.SVG_ID, '~')
    ...
    DBMS_OUTPUT.PUT_LINE('DW_MOLO_SLIPS: Inserted ' || p_inserted_count || ', Updated ' || p_updated_count || ' records');
