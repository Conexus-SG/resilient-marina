  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "API_USER"."SP_MERGE_STELLAR_BOOKINGS" 
IS
    v_inserted NUMBER := 0;
    v_updated NUMBER := 0;
...
BEGIN
    ...
    WHEN MATCHED THEN
        ...
        WHERE (
            ...
            NVL(tgt.CREATOR_ID, -999) <> NVL(src.CREATOR_ID, -999) OR
            ...
            NVL(tgt.CC_PREAUTH_ID, '~NULL~') <> NVL(src.CC_PREAUTH_ID, '~NULL~') OR
            NVL(tgt.CC_PREAUTH_AMOUNT, -999.999) <> NVL(src.CC_PREAUTH_AMOUNT, -999.999) OR
            NVL(tgt.CC_CONNECT_TYPE, '~NULL~') <> NVL(src.CC_CONNECT_TYPE, '~NULL~') OR
            NVL(tgt.CC_CONNECT_ID, '~NULL~') <> NVL(src.CC_CONNECT_ID, '~NULL~') OR
            NVL(tgt.ACCESSORIES_CUSTOM_PRICE, -999.999) <> NVL(src.ACCESSORIES_CUSTOM_PRICE, -999.999) OR
            NVL(tgt.ACCESSORIES_TOTAL, -999.999) <> NVL(src.ACCESSORIES_TOTAL, -999.999) OR
            NVL(tgt.INSURANCE_AMOUNT, -999.999) <> NVL(src.INSURANCE_AMOUNT, -999.999) OR
            NVL(tgt.PETS, -999.999) <> NVL(src.PETS, -999.999) OR
            NVL(tgt.PARKING, -999.999) <> NVL(src.PARKING, -999.999) OR
            NVL(tgt.PARKING_OVERRIDE, -999) <> NVL(src.PARKING_OVERRIDE, -999) OR
            NVL(tgt.BOATS_TOTAL, -999.999) <> NVL(src.BOATS_TOTAL, -999.999) OR
            NVL(tgt.POS_TOTAL, -999.999) <> NVL(src.POS_TOTAL, -999.999) OR
            NVL(tgt.USE_CLUB_CREDITS, -999) <> NVL(src.USE_CLUB_CREDITS, -999) OR
            NVL(tgt.NO_SHOW_FEE, -999) <> NVL(src.NO_SHOW_FEE, -999) OR
            NVL(tgt.CANCELLATION_FEE, -999) <> NVL(src.CANCELLATION_FEE, -999) OR
            NVL(tgt.CLUB_FEES, -999.999) <> NVL(src.CLUB_FEES, -999.999) OR
            NVL(tgt.CLUB_FEES_OVERRIDE, -999) <> NVL(src.CLUB_FEES_OVERRIDE, -999) OR
            NVL(tgt.SUB_TOTAL, -999.999) <> NVL(src.SUB_TOTAL, -999.999) OR
            NVL(tgt.CONVENIENCE_FEE, -999.999) <> NVL(src.CONVENIENCE_FEE, -999.999) OR
            NVL(tgt.CONVENIENCE_FEE_WAIVED, -999) <> NVL(src.CONVENIENCE_FEE_WAIVED, -999) OR
            ...
            NVL(tgt.TAX_1_EXEMPT, -999) <> NVL(src.TAX_1_EXEMPT, -999) OR
            NVL(tgt.TAX_1_RATE_OVERRIDE, -999.999) <> NVL(src.TAX_1_RATE_OVERRIDE, -999.999) OR
            NVL(tgt.TAX_2, -999.999) <> NVL(src.TAX_2, -999.999) OR
            NVL(tgt.TAX_2_EXEMPT, -999) <> NVL(src.TAX_2_EXEMPT, -999) OR
            ...
            NVL(tgt.DEPOSIT_TOTAL, -999.999) <> NVL(src.DEPOSIT_TOTAL, -999.999) OR
            NVL(tgt.DEPOSIT_OVERRIDE, -999) <> NVL(src.DEPOSIT_OVERRIDE, -999) OR
            NVL(tgt.DEPOSIT_WAIVED, -999) <> NVL(src.DEPOSIT_WAIVED, -999) OR
            NVL(tgt.GRATUITY, -999.999) <> NVL(src.GRATUITY, -999.999) OR
            NVL(tgt.GRAND_TOTAL, -999.999) <> NVL(src.GRAND_TOTAL, -999.999) OR
            NVL(tgt.ADJUSTMENT_TOTAL, -999.999) <> NVL(src.ADJUSTMENT_TOTAL, -999.999) OR
            NVL(tgt.AMOUNT_PAID, -999.999) <> NVL(src.AMOUNT_PAID, -999.999) OR
            NVL(tgt.NOTES_CONTRACT, '~NULL~') <> NVL(src.NOTES_CONTRACT, '~NULL~') OR
            NVL(tgt.NOTES_FROM_CUSTOMER_CONTRACT, '~NULL~') <> NVL(src.NOTES_FROM_CUSTOMER_CONTRACT, '~NULL~') OR
            NVL(tgt.NOTES_FOR_CUSTOMER_CONTRACT, '~NULL~') <> NVL(src.NOTES_FOR_CUSTOMER_CONTRACT, '~NULL~') OR
            NVL(tgt.FRONTEND, '~NULL~') <> NVL(src.FRONTEND, '~NULL~') OR
            NVL(tgt.IS_ON_HOLD, '~NULL~') <> NVL(src.IS_ON_HOLD, '~NULL~') OR
            NVL(tgt.IS_LOCKED, '~NULL~') <> NVL(src.IS_LOCKED, '~NULL~') OR
            NVL(tgt.IS_FINALIZED, '~NULL~') <> NVL(src.IS_FINALIZED, '~NULL~') OR
            NVL(tgt.IS_CANCELED, '~NULL~') <> NVL(src.IS_CANCELED, '~NULL~') OR
            NVL(tgt.OVERRIDE_TURNAROUND_TIME, '~NULL~') <> NVL(src.OVERRIDE_TURNAROUND_TIME, '~NULL~') OR
            NVL(tgt.CANCELLATION_TYPE, '~NULL~') <> NVL(src.CANCELLATION_TYPE, '~NULL~') OR
            NVL(tgt.BYPASS_CLUB_RESTRICTIONS, '~NULL~') <> NVL(src.BYPASS_CLUB_RESTRICTIONS, '~NULL~') OR
            NVL(tgt.RENTERS_INSURANCE_INTEREST, '~NULL~') <> NVL(src.RENTERS_INSURANCE_INTEREST, '~NULL~') OR
            NVL(tgt.COUPON_ID, -999) <> NVL(src.COUPON_ID, -999) OR
            NVL(tgt.COUPON_TYPE, '~NULL~') <> NVL(src.COUPON_TYPE, '~NULL~') OR
            NVL(tgt.COUPON_AMOUNT, -999.999) <> NVL(src.COUPON_AMOUNT, -999.999) OR
            NVL(tgt.DISCOUNT_TOTAL, -999.999) <> NVL(src.DISCOUNT_TOTAL, -999.999) OR
            NVL(tgt.AGENT_ID, -999) <> NVL(src.AGENT_ID, -999) OR
            NVL(tgt.AGENT_NAME, '~NULL~') <> NVL(src.AGENT_NAME, '~NULL~') OR
            NVL(tgt.REFERRER_ID, '~NULL~') <> NVL(src.REFERRER_ID, '~NULL~') OR
            NVL(tgt.SAFETY_REMINDER, -999) <> NVL(src.SAFETY_REMINDER, -999) OR
            NVL(tgt.DELETED_ADMIN_ID, -999) <> NVL(src.DELETED_ADMIN_ID, -999) OR
            ...
            NVL(tgt.FINALIZED_AT, TO_TIMESTAMP('1900-01-01 00:00:00','YYYY-MM-DD HH24:MI:SS')) <> NVL(src.FINALIZED_AT, TO_TIMESTAMP('1900-01-01 00:00:00','YYYY-MM-DD HH24:MI:SS')) OR
    ...
    DBMS_OUTPUT.PUT_LINE('DW_STELLAR_BOOKINGS: ' || v_inserted || ' inserted, ' || v_updated || ' updated');
